...
export default function TableWithSWR({ fallbackData, query, currentPage }: TableWithSWRProps) {
  ...
  const columns: Column<RequestRecord>[] = [
    ...
    { header: "Closed Time", accessor: "closed_time" },
  ...
  return (
    <>
      <TableBody<RequestRecord>
        ...
        options={(_, record: RequestRecord) => <Actions record={record} mutate={mutate} />}
      />
      <div className="mt-5 flex w-full justify-center">
        <TablePagination currentPage={currentPage} totalPages={data.total_pages || 1} />
      </div>
    </>
  );
}
