 ...
 */
export default function UsersTable({ initialUsers, domainUsers, roles }: UsersTableProps) {
    ...
    return users.filter((user) => {
      ...
      const matchesRoles =
        ...
        selectedRoles.some((roleId) =>
          Object.keys(user.roles || {}).includes(roleId.toString())
        );
  ...
  // Update the handleSaveUser function to use useSWR
  const handleSave = (
    ...
    // Example API call: updateUserRoles(updatedUser.id, updatedUser.active, addedRoles, removedRoles);
  };
...
<Card className="h-full">
  ...
  <CardContent className="flex flex-col h-full">
    ...
    <div className="table-container" style={{ height: '80vh', overflowY: 'auto' }}>      <Table>
        <TableHeader className="sticky top-0 bg-background">
          <TableRow>
            <TableHead className="text-center align-middle">Username</TableHead>
            <TableHead className="text-center align-middle">Full Name</TableHead>
            <TableHead className="text-center align-middle">Title</TableHead>
            {roles.map((role) => (
          ...
          {filteredUsers.length > 0 ? (
            filteredUsers.map((user) => (
              <TableRow key={user.id} className={!user.active ? "opacity-60" : ""}>
                ...
                <TableCell className="text-center align-middle">{user.fullname}</TableCell>
                <TableCell className="text-center align-middle">
            ...
            <TableRow>
              <TableCell colSpan={4 + roles.length} className="text-center align-middle">
                No users found.
              </TableCell>
            </TableRow>
          )}
